...
// @Success 201 {int} models.Imagen
// @Failure 403 body is empty
// @router / [post]
func (c *ImagenController) Post() {
	var v models.Imagen
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddImagen(&v); err == nil {
...
func (c *ImagenController) GetAll() {
	...
	l, err := models.GetAllImagen(query, fields, sortby, order, offset, limit)
	if err != nil {
		c.Data["json"] = err.Error()
	} else {
		if l == nil {
...
func (c *ImagenController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.Imagen{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateImagenById(&v); err == nil {
			c.Data["json"] = map[string]interface{}{"success": true, "status": 200, "message": "Peticion existosa", "Data": v}
		} else {
...
func (c *ImagenController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteImagen(id); err == nil {
